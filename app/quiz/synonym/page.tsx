...
const buildQuestion = (word: Word, allWords: Word[]): Question => {
  const provided = word.quiz_synonyms;
  const fallbackCandidates = word.synonyms.length > 0 ? word.synonyms : [word.word];
  ...
  const synonymPool = allWords
    ...
    .flatMap((item) => (item.synonyms.length > 0 ? item.synonyms : [item.word]));

  const uniquePool = Array.from(new Set(synonymPool)).filter(
    (item) => item !== correct
  );
...
export default function SynonymQuizPage() {
    ...
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 px-4 py-10">
      ...
      <section className="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-soft">
        <div className="flex items-baseline justify-between">
          ...
          <div className="text-xs font-medium text-slate-500">類義語4択</div>
        </div>

        <div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
          次の語の類義語として最も近いものを選んでください。
        </div>
      </section>
